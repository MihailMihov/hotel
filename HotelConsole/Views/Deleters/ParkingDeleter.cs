...
public class ParkingDeleter : Deleter
{
    ...
    public ParkingDeleter(ICollection<Parking> parkings) : this()
    {
        ...
        if (!AnsiConsole.Confirm($"Are you sure that you want to delete {targetParkingNames.Count} parkings?"))
        {
            return;
        }
        
        foreach (var parkingName in targetParkingNames)
        {
            ParkingIds.Add(parkings.First(b => b.Name == parkingName).Id);
        }
    }
}